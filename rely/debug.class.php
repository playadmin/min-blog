...
class debug
{
    ...
    public static function GetJsonDebug($level = null)
    {
        ...
        if (2 < $level) {
            method_exists('\\libs\\view', 'GetParams') && self::$errs[1150] = \libs\view::GetParams();
    ...
    {
        $runtime = round(1000 * (microtime(true) - MTIME), 3) . 'ms';
...
EOT;
    ...
    {
        // if (!(is_file($file = $GLOBALS['ZPHP_MAPPING']['libs'] . 'view.class.php') && require $file) || !class_exists('\libs\view', false) || !$params = \libs\view::GetParams()) {
        //     return false;
        // }
        if (!$params = method_exists('\\libs\\view', 'GetParams') ? \libs\view::GetParams() : false) {
            return false;
        }
