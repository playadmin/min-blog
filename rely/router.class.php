...
class router
{
    const MOMENT = 'before-router';
    private static $MOD = 0,
    $IS_MODULE = 0,
    $ROUTER = [],
    $FORMAT = [],
    $APP_ISMODULE,
    ...
    private static function router(string $name = '')
    {
        $name || $name = APP_NAME;
        $path = P_ROOT . "app/{$name}/";
        if (!$router = is_file($file = "{$path}config/router.php") ? require $file : false) {
            return false;
        }
    ...
    private static function getIsmodule(string $app)
    {
        if ($app === APP_NAME) {
    ...
    private static function getModuleRouter(string $m, string $name = '')
    {
        $name || $name = APP_NAME;
        $M = "+{$m}";
    ...
    private static function format(string $name, $m)
    {
        $name || $name = APP_NAME;
        $key = "{$name}-{$m}";
    ...
    private static function getUf($path)
    {
        ...
        if ('.php' === substr($arr[0], -4)) {
            ...
            if ($ismodule = self::getIsmodule($uf['app'][1])) {
                if (4 !== count($arr)) {
    ...
    private static function getInPath($info, $param = false)
    {
        if (isset($info['app'])) {
            $php = $info['app'][0];
            $app = $info['app'][1];
    ...
    public static function U0($path, $args)
    {
        $Q = self::getUf($path);
    ...
    public static function U1($path, $args)
    {
    ...
    public static function U2($path, $args)
    {
        $info = self::getUf($path);
    ...
    public static function Url($path, array $args = [], $mod = null)
    {
        isset($mod) || $mod = $GLOBALS['ZPHP_CONFIG']['ROUTER']['mod'] ?? self::$MOD;
        if (is_array($mod)) {
            return self::U1($path, $args, $mod);
        }
