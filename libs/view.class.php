...
class view
{
    ...
    public static function GetTpl(string $name, $F = false)
    {
        if (!$name) {
            ...
            if (defined($arr[0]) && $file = rtrim(constant($arr[0]), '/')) {
                ...
                switch (count($arr)) {
                    case 1:
                        $file = P_THEME . ROUTE['ctrl'] . "/{$info['fullname']}";
                        break;
                    default:
                        $file = P_THEME . $info['fullname'];
                }
            }
        }
        if (!is_file($file)) {
    ...
    private static function getBlock($file, $key)
    {
        ...
        DEBUGER && 2 < $GLOBALS['ZPHP_CONFIG']['DEBUG']['level'] && debug::setMsg(1140, $file);
    ...
    public static function Fetch(string $name = '')
    {
        ...
        DEBUGER && 2 < $GLOBALS['ZPHP_CONFIG']['DEBUG']['level'] && debug::setMsg(1140, $tpl);
